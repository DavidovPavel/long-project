    ...
    public class UnhandledExceptionFilterAttribute : ExceptionFilterAttribute
    {
       #region UnhandledExceptionFilterAttribute()
        ...
        private static Func<Exception, HttpRequestMessage, HttpResponseMessage> DefaultHandler = (exception, request) =>
        {
            ...
            //var response = request.CreateErrorResponse(HttpStatusCode.InternalServerError, exception);
            response.ReasonPhrase = "Internal Error";  //через данный атрибут невозможно передавать значение в UTF-8 кодировке (используется только ISO-8859-1)
