define('result.docs.add', ['i18n!nls/resources.min', 'g/tree', 'g/ejRTEView'], function (Resources, Tree, Editor) {

    return Mn.View.extend({

        template: templates['dialog-creating-document'],
        ...
        onRender: function () {
            ...
            this.showChildView('tree', new Tree({ collection: new Backbone.Collection, node: { checkbox: false }, branch: { pid: parseInt(this.model.get("branch").id) } }));
            this.getChildView('tree').collection.url = '/api/Tree';
            this.getChildView('tree').collection.fetch({
                reset: true,
                success: function () {
